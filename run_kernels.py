...
if __name__ == "__main__":
 ...
 if mode == "glove":
     ...
     X,Y,X_dev,Y_dev = load_synthetic(normalize=False, mode=mode)
 ...
 with open("interim/glove{}/kernel/multiple/multiple-kernels.pickle".format(run_id), "wb") as f:
            ...
            for i,gamma in enumerate(kernel2params[kernel_type]["gammas"]):
                for j,degree in enumerate(kernel2params[kernel_type]["degrees"]):
                    for k,alpha in enumerate(kernel2params[kernel_type]["alphas"]):
                        ...
                        # Run adversarial game on the kernel features    
                        ws, advs, best_adv, best_score = solve_fantope_relaxation(X_kernel, Y, X_dev_kernel, Y_dev,d=RANK,init_beta=None,device=args.device, out_iters=35000, in_iters_adv=1, in_iters_clf=1, batch_size = 256, epsilon=0.001, noise_std=1e-7, lr=0.08, weight_decay=1e-4)
         
                        U,D = get_svd(best_adv)
                        U = U.T
                        W = U[-RANK:] 
                        P = np.eye(X_kernel.shape[1]) - W.T@W
                        with open("interim/glove{}/kernel/P.{}.pickle".format(run_id, params_str), "wb") as f:
                            pickle.dump(P, f)
                        ...
                        with open("interim/glove{}/kernel/projected/X.proj.{}.pickle".format(run_id,params_str), "wb") as f:                   
                            pickle.dump(X_kernel_proj, f)
                        with open("interim/glove{}/kernel/projected/X_dev.proj.{}.pickle".format(run_id,params_str), "wb") as f:               
                            pickle.dump(X_dev_kernel_proj, f)
                        with open("interim/glove{}/kernel/projected/X_test.proj.{}.pickle".format(run_id,params_str), "wb") as f:               
                            pickle.dump(X_test_kernel_proj, f)
                 
                        with open("interim/glove{}/kernel/nystrom/X.{}.pickle".format(run_id,params_str), "wb") as f:                       
                            pickle.dump(X_kernel, f)
                        with open("interim/glove{}/kernel/nystrom/X_dev.{}.pickle".format(run_id,params_str), "wb") as f:                       
                            pickle.dump(X_dev_kernel, f)
                        with open("interim/glove{}/kernel/nystrom/X_test.{}.pickle".format(run_id,params_str), "wb") as f:                       
                        ...
                        for random_try in range(1):
                            ...
                            with open("interim/glove{}/kernel/preimage/MLP.{}.pickle".format(run_id, params_str), "wb") as f:
                                pickle.dump((mlp.cpu(), error, mean_norm_normalized), f)
                        
                            with open("interim/glove{}/kernel/preimage/Z.{}.pickle".format(run_id, params_str), "wb") as f:                       
                                pickle.dump((Z,error, mean_norm_normalized, best_score), f)
 
                                with open("interim/glove{}/kernel/preimage/Z_dev.{}.pickle".format(run_id,params_str), "wb") as f:                       
                                    pickle.dump((Z_dev, error, mean_norm_normalized, best_score), f)
                                with open("interim/glove{}/kernel/preimage/Z_test.{}.pickle".format(run_id,params_str), "wb") as f:                       
