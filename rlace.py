...
def solve_fantope_relaxation(X,Y,X_dev,Y_dev, d=1,init_beta=None,device="cpu", out_iters=50000, in_iters_adv=1,
    ...
    Y_dev_torch = torch.tensor(Y_dev).float().to(device)
    ...
    with open("accs.txt".format(d), "w") as f:
     ...
     for i in pbar:
        ...
        if i % evalaute_every == 0:
            ...
            score = get_score(X_dev,Y_dev, advs[-1], d)
